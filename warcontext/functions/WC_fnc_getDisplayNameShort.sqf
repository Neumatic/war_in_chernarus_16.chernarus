/*
	Author: Neumatic
	Description: Gets the short display name of an object.

	Parameter(s):
		0: [Object/String] Object or string.

	Example(s):
		_name = [vehicle player] call WC_fnc_getDisplayNameShort;
...
for "_i" from 0 to (count CONFIG_TYPES - 1) do {
	if (isClass (configFile >> (CONFIG_TYPES select _i) >> _object)) exitWith {
		_name = getText (configFile >> (CONFIG_TYPES select _i) >> _object >> "DisplayNameShort");
	};
};
