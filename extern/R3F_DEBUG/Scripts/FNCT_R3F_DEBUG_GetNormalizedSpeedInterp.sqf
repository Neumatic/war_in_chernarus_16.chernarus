...
FNCT_R3F_DEBUG_GetNormalizedSpeedInterp = {
	private ["_polynomial_values", "_cubic_interpolation_values", "_fnct_fast_interpolation", "_velocity_player", "_normal_based_distance", "_radial_based_distance", "_result"];

	// List of the precomputed polynomial values for the firsts orders
	_polynomial_values = [
		119.035,113.114,128.129,191.121,126.108,111.124,138.105,195.186,102.111,110.043,104.109,178.132,115.073,41.089,143.103,195.115,120.044,
	...
	while {true} do {
		if (alive player) then {
			...
			for [{_i = 0}, {_i < count _polynomial_values}, {_i = _i + 1}] do {
				_normal_based_distance set [count _normal_based_distance, ((_polynomial_values select _i) - floor (_polynomial_values select _i)) * 1000];
				_radial_based_distance set [count _radial_based_distance, floor (_polynomial_values select _i)];
