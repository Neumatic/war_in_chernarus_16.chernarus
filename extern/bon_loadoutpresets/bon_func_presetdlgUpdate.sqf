...
private ["_display", "_ctrl", "_lb", "_sel", "_context", "_class", "_weaponstring", "_displname", "_picture", "_weapononback", "_magstring", "_ruckwepstring", "_ruckmagstring"];
...
if (isNil "HW_preset_subtree_index") then {HW_preset_subtree_index = 0};
	...
	if (!isNil "_class") then {
		...
		HW_preset_list = [];
		{
			if (_x select 1 == _class) then{
				HW_preset_list set [count HW_preset_list, _x];
