...
private [
	"_i","_e","_npc","_vehicles","_exit","_membertypes","_vehicletypes","_sharedenemy","_friendside","_enemyside","_friendlytanks","_closeenough",
	"_nomove","_minreact","_buildingdist","_makenewtarget","_wait","_respawnmax","_issoldier","_initpos","_orgMode","_orgSpeed","_reinforcement","_currPos",
	"_nowp","_try","_bld","_bldpos","_posinfo","_unitsIn","_targetPos","_index","_wp","_gothit","_targets","_NearestEnemy","_attackPos","_dist","_newattackPos",
	"_lastknown","_opfknowval","_target","_targetsnear","_newtarget","_dist3","_timeontarget","_targetdead","_flankdir","_fightmode","_react","_speedmode",
	"_Behaviour","_waiting","_unitpos","_pursue","_Mines","_mineposition","_dir1","_roads","_ambushed","_currcycle","_wctemp","_ambush","_prov","_lastdist",
	"_GetOutDist","_lastcurrpos","_timeout","_pause","_fixedtargetPos","_speed","_landing","_heli","_flankdist","_newflankAngle","_fldest","_fldest2",
	"_fldestfront","_dist1","_dist2","_flankPos","_targettext","_wptype","_wpformation","_enemytanksnear","_enemytanknear","_friendlytanksnear","_newpos",
	"_targetdist","_destname","_lastreact","_fm","_supressed","_targetX","_targetY","_dir2","_avoidPos","_frontPos","_dirf1","_dirf2","_flankPos2","_rnd",
	"_incar","_inheli","_inboat","_reinforcementsent","_centerpos","_centerX","_centerY","_areasize","_rangeX","_rangeY","_areadir","_lastpos","_a","_dp",
	"_drydist","_swimming","_vehicle","_cargo","_flyInHeight","_GetIn_NearestVehicles","_lastwptype","_loop","_currX","_currY","_trackername","_UCthis",
	"_grpid","_GetOut","_GetIn","_rfid","_side","_surrended","_ambushdist","_ambushwait","_respawn","_lead","_safemode","_initstr","_obj","_grpidx","_grpname",
	"_areamarker","_showmarker","_group","_members","_isman","_iscar","_isboat","_isplane","_friends","_enemies","_area","_cosdir","_sindir","_mindist",
	"_orgPos","_orgDir","_orgWatch","_lastmove1","_lastmove2","_maxmove","_moved","_hitPos","_sin0","_sin90","_cos90","_sin270","_cos270","_nBuilding",
	"_nBuildingt","_objsflankPos2","_dir3","_dd","_planta","_gunner","_driver","_fortify","_template","_nowpType","_ambushtype","_orignowp","_nofollow",
	"_shareinfo","_cycle","_noslow","_spawned","_zoneempty","_sharedist"
];
...
// Vehicle will not be in units so must set manually
...
while {_loop} do {
	...
	// No units left exit the loop
	if ({alive _x} count units _group == 0) exitWith {};

	// Did anybody in the group get hit?
	{
	...
	_npc = leader _group;
	if (isNull _npc || {isPlayer _npc}) exitWith {};
	...
	if (_issoldier && !_exit) then {
		...
		_targets = missionNamespace getVariable [format ["KRON_targets%1", _sharedenemy], []];
		...
		if (_shareinfo == "SHARE") then {
			...
			if (KRON_UPS_comradio == 2) then {
				...
				// We have a close target alive do not search another
		...
		if (!isNull _target && {alive _target}) then {
			...
			if (!isNil "_newattackPos") then {
				...
				(units _group) lookAt _attackPos;
			};
		};
		...
		if (alive _npc && {!_nowp}) then {
			if (!_isman || {(vehicle _npc != _npc && {!_inboat} && {!((vehicle _npc) isKindOf "StaticWeapon")})}) then {
				...
				if (!_inheli) then {
					...
					if (_gothit || _dist <= (_closeenough * 1.5) ||
						((_lastcurrpos select 0 == _currpos select 0) && (_lastcurrpos select 1 == _currpos select 1) && moveToFailed (vehicle _npc))
...
//**********************************************************************************************************************
		if (_pursue) then {
			...
			_npc = leader _group;
			if (isNull _npc || {isPlayer _npc}) exitWith {_exit = true};
			...
			{
				if (_forEachIndex != _grpid && {format ["%1", _x] != "[0,0]"}) then {
			...
			{
				if (_forEachIndex != _grpid && {!isNull _x}) then {
			...
			if (_fightmode != "walk") then {
				if (_timeontarget > KRON_UPS_alerttime && {count _targets <= 0 && {(isNull _target || {!alive _target} || {captive _target})}}) then {
					...
					if (_rfid > 0) then {
						...
						missionNamespace setVariable [format ["KRON_UPS_reinforcement%1", _rfid], false];
					};
...
//**********************************************************************************************************************
					...
					if (count _unitsIn > 0) then {
...

						{
							if ((vehicle _x) isKindOf "Air") then {
								// Moving heli for avoiding stuck
			...
			if (!_GetIn_NearestVehicles) then {
				if (_wptype == "HOLD" && {vehicle _npc == _npc && {(_fortify || random 100 < 60)}}) then {
					...
					if (_wptype != "HOLD" && {vehicle _npc == _npc && {random 100 < 90}
						&& {_npc == vehicle _npc} && {_dist <= _closeenough}}
					) then {
	...
	// If in heli calculations must done faster
