...
tpwcas_fnc_main_loop =
{
	...
	while {true} do
	{
		{ //foreach start
			...
			if ( (_unitCheck) && (vehicle _x == _x) && (lifestate _x == "ALIVE") && (side _x != civilian) ) then
			{
				_unit  = _x;
				...
				if (!isNull _unit) then
				{
					...
					{
						if !( _unit getVariable "tpwcas_stance" == "auto") then
						{
							...
							//RESET UNIT STANCE
							if (!isNil {_unit getvariable "tpwcas_stance"}) then
							{
								_stance = _unit getvariable "tpwcas_stance";
						...
						else
						{
							...
							switch ( _unit getvariable "tpwcas_supstate" ) do
							{
								case 2: //IF ENEMY BULLETS NEAR PLAYER
								{
									...
									{
										addCamShake [0.5, 3 + (random 3), 2.5];
									};
								};
			...
			{
				//if civilian
				if ((side _x == civilian) && (vehicle _x == _x) && (lifestate _x == "ALIVE") && !(isPlayer _x)) then
				{
					_unit  = _x;
