...
R3F_REV_fil_exec_attente_reanimation = objNull spawn {
	private ["_position_avant_mort", "_direction_avant_mort", "_chargeurs_avant_mort", "_armes_avant_mort", "_hasruck", "_hasruckace", "_ruckmags", "_ruckweapons", "_weapononback", "_backpack", "_rucktype", "_sacados_avant_mort", "_position"];
	...
	if (!isNil "R3F_REV_effet_video_flou") then {
	...
	if (R3F_REV_nb_reanimations > 0) then {
		...
		[localize "STR_R3F_REV_attente_reanimation"] call BIS_fnc_dynamicText;
		...
		while {((damage player > 0.7) && {!(wcteamscore < wcscorelimitmin)} && {!(wcteamscore > wcscorelimitmax)})} do {
			sleep 1;
		};
		...
		if (R3F_REV_nb_reanimations > 0) then {
			if (R3F_REV_nb_reanimations > 1) then {
				[format [localize "STR_R3F_REV_nb_reanimations_plusieurs", R3F_REV_nb_reanimations]] call BIS_fnc_dynamicText;
			} else {
				[format [localize "STR_R3F_REV_nb_reanimations_une", R3F_REV_nb_reanimations]] call BIS_fnc_dynamicText;
			};
		}
		else {
			[localize "STR_R3F_REV_nb_reanimations_zero"] call BIS_fnc_dynamicText;
		};
		...
		[localize "STR_R3F_REV_plus_de_reanimation"] call BIS_fnc_dynamicText;
		...
		sleep 5;
		[localize "STR_R3F_REV_respawn_camp"] call BIS_fnc_dynamicText;
