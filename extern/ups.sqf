...
private [
	"_UCthis", "_i", "_e", "_exit", "_onroof", "_areamarker", "_centerpos", "_centerX", "_centerY", "_rangeX", "_rangeY",
	"_areadir", "_areaname", "_areatrigger", "_showmarker", "_getAreaInfo", "_areasize", "_obj", "_npc", "_grpidx",
	"_grpname", "_members", "_isman", "_iscar", "_isboat", "_isplane", "_issoldier", "_friends", "_enemies", "_sharedenemy",
	"_named", "_npcname", "_cosdir", "_sindir", "_mindist", "_orgMode", "_orgSpeed", "_speedmode", "_currPos", "_orgPos",
	"_orgWatch", "_orgDir", "_avoidPos", "_flankPos", "_attackPos", "_dist", "_lastdist", "_lastmove1", "_lastmove2",
	"_maxmove", "_moved", "_damm", "_dammchg", "_lastdamm", "_timeontarget", "_fightmode", "_fm", "_gothit", "_hitPos",
	"_react", "_lastdamage", "_lastknown", "_opfknowval", "_sin90", "_cos90", "_sin270", "_cos270", "_closeenough",
	"_pause", "_nomove", "_nofollow", "_shareinfo", "_usetrigger", "_cycle", "_initpos", "_noslow", "_try", "_bld",
	"_bldpos", "_posinfo", "_track", "_trackername", "_destname", "_markerDot", "_markerobj", "_markercolor", "_markertype",
	"_deletedead", "_mincopies", "_maxcopies", "_isMan", "_copies", "_initstr", "_nameprefix", "_unittype", "_rnd", "_grp",
	"_lead", "_c", "_newunit", "_nul", "_zoneempty", "_trgside", "_trgname", "_flgname", "_markerhide", "_markershow",
	"_makenewtarget", "_newpos", "_targetPos", "_swimming", "_waiting", "_loop", "_currcycle", "_newdamage", "_currX",
	"_currY", "_others", "_maxknowledge", "_knows", "_pursue", "_accuracy", "_offsx", "_offsY", "_offsX", "_targetX",
	"_targetY", "_dir1", "_dir2", "_dir3", "_dd", "_relUX", "_relUY", "_relTX", "_relTY", "_sinU", "_cosU", "_sinT",
	"_cosT", "_avoidX", "_avoidY", "_flankX", "_flankY", "_totmove", "_tries", "_roadlist", "_road", "_drydist", "_a",
	"_dp", "_cont", "_find_target"
];
...
if (isNil("WC_KRON_UPS_INIT")) then {
	...
	// Misc
	WC_KRON_getArg = {private["_cmd","_arg","_list","_a","_v"]; _cmd=_this select 0; _arg=_this select 1; _list=_this select 2; _a=-1; {_a=_a+1; _v=format["%1",_list select _a]; if (_v==_cmd) then {_arg=(_list select _a+1)}} forEach _list; _arg};
	...
	WC_KRON_KnownEnemy=[objNull,objNull,objNull]; // enemy information is shared between different groups
...
missionNamespace setVariable [format ["WC_KRON_UPS_%1", _npcname], 1];
...
if (_deletedead>0) then {
	{_x addEventHandler['killed',format["[_this select 0,%1] spawn WC_KRON_deleteDead",_deletedead]]} forEach _members;
};
...
if (_usetrigger!="NOTRIGGER") then {
	_trgside = switch (side _npc) do { case west: {"WEST"}; case east: {"EAST"}; case resistance: {"GUER"}; default {"CIV"};};
...
while {_loop} do {
	...
	if ((_issoldier) && !(_exit)) then {
		...
		if (_shareinfo=="SHARE") then {
			_others = (_currPos nearObjects ["CAManBase", SHAREDIST]) - _members;
			{
		...
		_find_target = _npc findNearestEnemy _npc;
		if (!isNull _find_target) then {
			if (alive _find_target) then {
				_knows = _npc knowsAbout _find_target;
				if (_knows > 0.5) then {
					WC_KRON_KnownEnemy set [_sharedenemy, _find_target];
		...
		if (_waiting<=0) then {
			...
			if (_makenewtarget) then {
				if ((_nomove=="NOMOVE") && (_timeontarget>ALERTTIME)) then {
					if (([_currPos,_orgPos] call WC_KRON_distancePosSqr)<_closeenough) then {
						_newpos = false;
					} else {
						_targetPos=_orgPos;
					};
	...
	_cont = missionNamespace getVariable format ["WC_KRON_UPS_%1", _npcname];
